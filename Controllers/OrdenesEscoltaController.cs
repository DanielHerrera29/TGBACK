...
public sealed class OrdenesEscoltaController : ControllerBase
{
    ...
    public async Task<IActionResult> Enviar(string id, [FromBody] EnviarOrdenEscoltaDto dto, CancellationToken cancellationToken)
    {
        ...
        var otp = HttpContext.RequestServices.GetRequiredService<FirmaOtpService>();
        if (!await otp.Authorized(session.UserId, id))
            return Conflict(new { code = "FIRMA_NO_VERIFICADA", error = "Valide el código del arquitecto para habilitar la firma de esta orden." });
        ...
        try
        {
            ...
            correoIniciado = true;
            var mensaje = await _db.ConstruirMensajeOrdenEscoltaAsync(id, orden.NumeroVisible);
            if (!string.IsNullOrWhiteSpace(dto.NombreArquitecto) || !string.IsNullOrWhiteSpace(dto.CorreoArquitecto))
                mensaje += $"\n\nArquitecto: {dto.NombreArquitecto}\nCorreo verificado: {dto.CorreoArquitecto}";
            await _email.EnviarOrdenEscoltaAsync(orden.Consecutivo, pdf, cancellationToken, orden.CodigoOrden, mensaje);
